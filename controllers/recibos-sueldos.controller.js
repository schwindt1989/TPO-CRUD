...
async function GetAllReciboSueldos(req, res) {
    const sql = 'SELECT rs.id, rs.mes, rs.anio, rs.sueldo, e.nombre, c.nombre as "cateogria" from recibos_sueldos rs left join empleados e on rs.empleado_id = e.idleft join categorias c on e.categoria_id = c.id';
    const result = await db.query(sql)
    res.json(result.rows);
}

async function GetOneReciboSueldo(req, res) {
    const sql = 'SELECT rs.id, rs.mes, rs.anio, rs.sueldo, e.nombre, c.nombre as "cateogria" from recibos_sueldos rs left join empleados e on rs.empleado_id = e.idleft join categorias c on e.categoria_id = c.id WHERE rs.id = $1';
